...
- **Hero background slideshow showed placeholder image on load**: Root cause was client-side fetch waterfall and missing preload. Fixed via server-side fetch in `hero-slides-server.ts` and `<link rel="preload">` in `layout.tsx`. On error, returns `[]` rather than fallback stock images. Before touching hero slideshow code: confirm data is still fetched server-side in layout.
