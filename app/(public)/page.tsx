...
import { fetchDepartmentsServer } from "@/lib/data/departments-server";

// Revalidate every 60s — stats stay fresh without a full rebuild
export const revalidate = 60;

export default async function HomePage() {
  // Run both server fetches in parallel — no sequential waterfall
  const [{ activeMembers, eventsOrganised }, initialDepartments] = await Promise.all([
    getLivePublicStats(),
  ...
  return (
    <div className="flex flex-col gap-16 md:gap-24 pb-24 overflow-hidden bg-transparent">
      ...
      <EventsShowcase />
      <DepartmentsShowcase initialDepartments={initialDepartments} />
