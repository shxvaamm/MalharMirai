...
export function HeroBackgroundSlideshow({
  ...
  return (
    ...
    >
      ...
      <div
        suppressHydrationWarning
        className={`absolute inset-0 h-full w-full ${opacityClassName}`}
      >
        {activeSlides.map((slide, index) => {
          ...
          return (
            <div
              key={slide.id}
              className="absolute inset-0 slide-layer overflow-hidden"
              style={{
      ...
      <div className="absolute inset-0 bg-gradient-to-b from-black/15 via-black/45 to-black [background:radial-gradient(ellipse_at_center,transparent_30%,#000_90%),linear-gradient(to_bottom,rgba(0,0,0,.15),rgba(0,0,0,.45),#000)]" />
    </div>
  );
}
